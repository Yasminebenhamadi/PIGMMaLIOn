...
gmm_diag<eT>::learn
  ...
  if(em_iter > 0)
    ...
    const bool status = em_iterate(X, em_iter, var_floor_actual, print_mode, percent_sum,final_loglikelihood,nb_iter);
  ...
  if(dist_mode == eucl_dist)
    {
      ...
      for(uword i=0; i<X_n_cols; ++i)
        ...
        for(uword g=0; g<N_gaus; ++g)learn
          {
...
inline
bool
gmm_diag<eT>::em_iterate(const Mat<eT>& X, const uword max_iter, const eT var_floor, const bool verbose, eT& percent_sum, eT& final_loglikelihood, eT& nb_iter)
  {
  arma_extra_debug_sigprint();
  ...
  Col<eT>          t_percent_sum(n_threads);
  ...
  for(uword iter=1; iter <= max_iter; ++iter)
    ...
    em_update_params(X, boundaries, t_acc_means, t_acc_dcovs, t_acc_norm_lhoods, t_gaus_log_lhoods, t_progress_log_lhood, t_percent_sum);
    
    em_fix_params(var_floor);
    
    const eT new_avg_log_p = accu(t_progress_log_lhood) / eT(t_progress_log_lhood.n_elem);

    const eT new_avg_percent = accu(t_percent_sum) / eT(t_percent_sum.n_elem);
    
    percent_sum=new_avg_percent;
    final_loglikelihood=new_avg_log_p;
    ...
    if(verbose)
      ...
      if(iter==max_iter){
        get_cout_stream() << "   percent: ";
        get_cout_stream().unsetf(ios::fixed);
        //get_cout_stream().setf(ios::scientific);
        get_cout_stream() << new_avg_percent;
      }
      ...
      get_cout_stream() << "   diff: ";
      //get_cout_stream().unsetf(ios::scientific);
      get_cout_stream().setf(ios::scientific);
      get_cout_stream() << std::abs(old_avg_log_p - new_avg_log_p)<< '\n';
      get_cout_stream().flush();
      }
    ...
    if(std::abs(old_avg_log_p - new_avg_log_p) <= Datum<eT>::eps)  { 
...
gmm_diag<eT>::em_update_params
  ...
  // em_generate_acc() is the "map" operation, which produces partial accumulators for means, diagonal covariances and hefts
    ...
    for(uword t=0; t<n_threads; t++)
      ...
      eT&      percent_sum = t_percent_sum[t];
      
      em_generate_acc(X, boundaries.at(0,t), boundaries.at(1,t), acc_means, acc_dcovs, acc_norm_lhoods, gaus_log_lhoods, progress_log_lhood,percent_sum);
      }
    }
  #else
    {
    em_generate_acc(X, boundaries.at(0,0), boundaries.at(1,0), t_acc_means[0], t_acc_dcovs[0], t_acc_norm_lhoods[0], t_gaus_log_lhoods[0], t_progress_log_lhood[0], t_percent_sum[0]);
    }
  #endif
...
gmm_diag<eT>::em_generate_acc
  ...
  percent_sum/=eT((end_index - start_index) + 1);
  percent_sum=1-percent_sum;
